...
def count_errors(initial_state, retrieved_state):
    ...
    return(equality_values.size-count)
...
def test_memory(METHOD, TEST, mfccs_vectors=None, U = 0, N = 100, n = 15, g = 100, p = 0.1, d=0.01, SEED = 27):
    ...
    rng = np.random.default_rng(seed=SEED)
    for v in V_test:
        idxs = rng.permutation(N)[:int(N*d)]
        v[idxs] = np.absolute(v[idxs]-1)
...
#             a=2*p-1
    ...
    if TEST == "errors" or TEST == "distance":
        ...
        for (j, noisy_state) in enumerate(V_test):
            ...
            print("Memory is: ", memory_state, "\n Distorted is: ", noisy_state)
            print("Retrieved state: ", memoryview(retrieved_state).tolist())
            print("Sum of all activations is: ", np.sum(memoryview(retrieved_state).tolist()))
            results["k"].append(k)
            if TEST == "errors":
                counts = count_errors(memory_state, retrieved_state)
                results["errors"].append(counts)
...
#             retrieved_state, k = retrieve_memory(T_I, noisy_state, U=U_eff)
#             # print("Retrieved state: ", memoryview(retrieved_state).tolist())
#             print(memory_state-retrieved_state)
#             results["k"].append(k)
#             if TEST == "errors":
#                 counts = count_errors(memory_state, retrieved_state)
#                 results["errors"].append(counts)
...
# results = test_memory("amit", "errors", mfccs_vectors=None, U = 4, N = 100, n = 15, g = 500, p = 0.1, d = 0.5, SEED = 27)