...
def test_memory(METHOD, TEST, mfccs_vectors=None, U = 0, N = 100, n = 15, g = 100, p = 0.1, d=0.01, SEED = 27):
    ...
    mask = sparsify_matrix(N)
    # TRAIN SYSTEM
    if METHOD == "default":
        T_I = initialize_network(N, 0.5, mask, V_train) # no inhibition
        U_eff = np.float(U)
    elif METHOD == "sparsity":
        T_I = initialize_network(N, p, mask, V_train) # no inhibition
        U_eff = np.float(U)
    elif METHOD == "amit":
        T = initialize_network(N, p, mask, V_train)            
#             a=2*p-1
